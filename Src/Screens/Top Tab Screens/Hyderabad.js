...
export default class HyderabadNews extends Component {
    constructor(props) {
        super(props);
        ...
        fetch(BaseUrl + CategoryUrl + Hyderabad).then((response) => response.json())
            .then(responseJson => {
                ...
                this.setState({ HyderabadData: responseJson, isLoading: true }, () => {
                    // console.log("HyderabadList data===="+ JSON.stringify(this.state.HyderabadData.data[0].link))
                });
            })
            .catch((error) => {
                console.error(error);
    ...
    // share function
    sharecall = () => {
        const Link_Url = ShareUrl + Hyderabad
        Share.share({
            message: Link_Url
        })
