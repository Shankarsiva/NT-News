...
export default class Category extends Component {
    ...
    componentDidMount() {
        ...
        fetch(BaseUrl + CategoryUrl + this.props.route.params.CatName).then((response) => response.json())
            .then(responseJson => {
                this.setState({ CategoryData: responseJson, isLoading: true }, () => {
                });
            })
            .catch((error) => {
                console.error(error);
    ...
    // share function
    sharecall = () => {
        const Link_Url = ShareUrl + this.props.route.params.CatName
        Share.share({
            message: Link_Url
        })
                    ...
                    <View>
                        {
                            ...
                            this.state.CategoryData.length != 0 && { isLoading: true } ?
