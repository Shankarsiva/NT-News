...
import { BaseUrl, CategoryUrl, National, ShareUrl, Sports } from "../../Utilities/Api/Urls";
...
export default class SportsNews extends Component {
    ...
    componentDidMount() {
        fetch(BaseUrl + CategoryUrl + Sports).then((response) => response.json())
            .then(responseJson => {
    ...
    // share function
    sharecall = () => {
        const Link_Url = ShareUrl + Sports
        Share.share({
            message: Link_Url
        })
 ...
 isMenu={true} isNotif={true}
                ...
                <SubHeader title={"స్పోర్ట్స్"} isMenu={false} isBook={false} isShare={true}
